...
COPY . .
RUN yarn install --frozen-lockfile && yarn build && rm -rf node_modules/ && yarn install --frozen-lockfile --production && node scripts/patch-oidc-provider-attestation-no-iss.cjs
